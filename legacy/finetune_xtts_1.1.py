...
XTTS_CANDIDATE_NAMES = [
    "tts_models--multilingual--multi-dataset--xtts",
...
def find_xtts_any():
    for d in TTS_CACHE_DIRS:
        for name in XTTS_CANDIDATE_NAMES:
            m = d / name
            if m.exists():
                cfg = m / "config.json"
                pth = m / "model.pth"
                if cfg.exists() and pth.exists():
                    return m, cfg, pth
...
def call_training(config_path: str, restore_path: str | None, cuda_devices: str):
    ...
    local_train = Path("/home/jacob/anaconda3/envs/tts_env/lib/python3.10/site-packages/TTS/bin/train_tts.py")
...
def main():
    ...
    model_dir, cfg_file, restore_path = find_xtts_any()
    if model_dir is None:
        print("ERROR: Could not find any XTTS model in TTS cache.")
        sys.exit(1)

    print("Using XTTS model at:", model_dir)
